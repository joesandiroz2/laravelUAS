<x-dashboard.layout>
    <div class="h-full p-6">
        ...
        <form action="/dashboard/products/{{ $produk->kode }}" method="POST" enctype="multipart/form-data" class="w-96 pb-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-4 lg:gap-6">
                <div>
                    ...
                    <input type="text" disabled name="kode" id="kode" required value="{{ $produk->kode }}" class="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none">
                </div>

                <div>
                    <label for="nama" class="block mb-2 text-sm text-gray-700 font-medium">Nama</label>
                    <input type="text" name="nama" id="nama" required value="{{ $produk->nama }}" class="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none">
                </div>

                <div>
                    <label for="deskripsi" class="block mb-2 text-sm text-gray-700 font-medium">Deskripsi</label>
                    <textarea name="deskripsi" id="deskripsi" required class="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none">{{ $produk->deskripsi }}</textarea>
                </div>

                <div>
                    <label for="harga" class="block mb-2 text-sm text-gray-700 font-medium">Harga</label>
                    <input type="number" name="harga" id="harga" value="{{ (int)$produk->harga }}" required class="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none">
                </div>
                ...
                <div>
                    ...
                    <div class="relative" data-hs-combo-box="">
                        <div class="relative">
                            <input class="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none" type="text" name="kategori" value="{{ $produk->kategori->nama }}" required data-hs-combo-box-input="">
                ...
                <div>
                    ...
                    <input type="file" name="gambar" id="gambar" accept="image/*" value="{{ str_replace('img/', '', $produk->gambar) }}" required class="block w-full border border-gray-200 shadow-sm rounded-lg text-sm focus:z-10 focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none
                        file:bg-gray-50 file:border-0
                        file:me-4
                        file:py-3 file:px-4">
                    <img id="preview" class="w-40 h-40 object-cover mt-4" src="/{{ $produk->gambar }}" alt="gambar hasil inputan">
                </div>
            </div>
            <!-- End Grid -->

<x-dashboard.layout>
    <div class="h-full p-6">
        ...
        <form action="/dashboard/orders/{{ $pesanan->id }}" method="POST" class="w-96">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-4 lg:gap-6">
                <div>
                    ...
                    <input type="datetime-local" name="tanggal" id="tanggal" required value="{{ $pesanan->tanggal }}" class="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none">
                </div>

                <div>
                    <label for="total_harga" class="block mb-2 text-sm text-gray-700 font-medium">Total Harga</label>
                    <input type="number" name="total_harga" id="total_harga" required value="{{ (int)$pesanan->total_harga }}" disabled class="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none">
                </div>
                ...
                <div>
                    ...
                    <div class="relative" data-hs-combo-box="">
                        <div class="relative">
                            <input required class="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none" type="text" name="status" value="{{ $pesanan->status->nama }}" data-hs-combo-box-input="">
            ...
            <input type="hidden" name="id_pengguna" value="{{ $pesanan->id_pengguna }}">

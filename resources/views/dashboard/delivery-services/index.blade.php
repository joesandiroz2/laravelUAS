<x-dashboard.layout>
    ...
    <div class="h-full p-6">
        <div class="flex justify-between my-4 px-6">
            ...
            <form action="/dashboard/delivery-services/create" method="GET">
                @csrf
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            @foreach($jasa2 as $jasa)
                                <tr class="odd:bg-white even:bg-gray-100 hover:bg-gray-100">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">{{ $jasa->kode }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $jasa->nama }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium flex">
                                        <form action="/dashboard/delivery-services/{{ $jasa->kode }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                        ...
                                        <a href="/dashboard/delivery-services/{{ $jasa->kode }}/edit" class="edt-item hover:bg-gray-200 rounded-full p-1">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487Zm0 0L19.5 7.125" />
                                            </svg>
                                        </a>
                                        <input type="hidden" value="{{ $jasa->kode }}">
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>

<x-dashboard.layout>
    <div class="h-full p-6">
        ...
        <form action="/dashboard/delivery-services/{{ $jasa->kode }}" method="POST" enctype="multipart/form-data" class="w-96 pb-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-4 lg:gap-6">
                <div>
                    ...
                    <input type="text" disabled name="kode" id="kode" required value="{{ $jasa->kode }}" class="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none">
                </div>

                <div>
                    <label for="nama" class="block mb-2 text-sm text-gray-700 font-medium">Nama</label>
                    <input type="text" name="nama" id="nama" required value="{{ $jasa->nama }}" class="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none">
                </div>
            </div>
            <!-- End Grid -->
